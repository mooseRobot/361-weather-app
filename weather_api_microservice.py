...
geo_response = requests.get(f"http://api.openweathermap.org/geo/1.0/direct?q={city}&limit={limit}&appid={API_KEY}")
...
weather_response_current = requests.get(f"https://api.openweathermap.org/data/2.5/forecast/daily?lat={lat}&lon={lon}&units=imperial&&cnt={3}&appid={API_KEY}")
...
epoch = datetime(1970, 1, 1)
d = datetime(int((str(future_date_first))[0:4]), int((str(future_date_first))[5:7]), int((str(future_date_first))[8::]))
d_1 = datetime(int((str(future_date_last))[0:4]), int((str(future_date_last))[5:7]), int((str(future_date_last))[8::]))
...
epoch = datetime(1970, 1, 1)
new_d = datetime(int((str(prev_date_first))[0:4]), int((str(prev_date_first))[5:7]), int((str(prev_date_first))[8::]))
...
weather_response_previous = requests.get(f"https://history.openweathermap.org/data/2.5/history/city?lat={lat}&lon={lon}&type=daily&units=imperial&start={int(past_1)}&end={int(past)}&appid={API_KEY}")
weather_response_future = requests.get(f"https://history.openweathermap.org/data/2.5/history/city?lat={lat}&lon={lon}&type=hour&units=imperial&start={int(future)}&end={int(future_1)}&appid={API_KEY}")
weather_response_current_updated = requests.get(f"https://api.openweathermap.org/data/2.5/forecast/daily?lat={lat}&lon={lon}&units=imperial&&cnt={3}&appid={API_KEY}")
